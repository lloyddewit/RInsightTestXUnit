...
public class RInsightTestXUnit
{
    ...
    public void TestGetLstLexemes()
    {
        ...
        var lstActual = RInsight.clsRScript.GetLstLexemes("a::b:::ba$c@d^e:ea%%f%/%g%*%h%o%i%x%j%in%k/l*m+n-o<p>q<=r>=s==t!=u!v&wx&&y|z||a2~2b->c0a->>d0123456789a<-1234567890<<-e0a1b2=345f6789");
        ...
        lstActual = RInsight.clsRScript.GetLstLexemes(",ae;af" + "\r" + "ag" + "\n" + "(ah)" + "\r\n" + "ai{aj}ak[al]al[[am]]_ao%>%|>.ap" + "%aq%.ar_2%asat%au_av.awax");
        Assert.Equal(lstExpected, lstActual);

        // test lexeme list - spaces
        lstExpected = new List<string>(new string[] { " ", "+", "ay", "-", " ", "az", "  ", "::", "ba", "   ", "%*%", "   ", "bb", "   ", "<<-", "    ", "bc", " ", "\r", "  ", "bd", "   ", "\n", "    ", "be", "   ", "\r\n", "  ", "bf", " " });
        lstActual = RInsight.clsRScript.GetLstLexemes(" +ay- az  ::ba   %*%   bb   <<-    bc " + "\r" + "  bd   " + "\n" + "    be   " + "\r\n" + "  bf ");
        Assert.Equal(lstExpected, lstActual);
        ...
        lstActual = RInsight.clsRScript.GetLstLexemes("\"a\"+\"bf\"%%\"bga\"%/%\"bgba\"%in%\"bgbaa\">=" + @"""~!@#$%^&*()_[] {} \|;:',./<>? "","" bgbaaa""" + "\r" + "\"bh\"" + "\n" + "\"bi\"" + "\r\n" + "\"bj\"{\"bk\"[[\"bl\"%>%\"bm\"%aq%\"bn\" +\"bn\"- \"bo\"  ::" + "\"bq\"   <<-    \"br\" " + "\r" + "  \"bs\"   " + "\n" + "    \"bt\"   " + "\r\n" + "  \"bu\" ");
        Assert.Equal(lstExpected, lstActual);

        // test lexeme list - comments
        lstExpected = new List<string>(new string[] { "#", "\n", "c", "#", "\n", "ca", "#", "\n", "+", "#", "\n", "%/%", "#", "\n", "%in%", "#", "\n", ">=", @"#~!@#$%^&*()_[]{}\|;:',./<>?#", "\n", " ", "#", "\n", "  ", @"#~!@#$%^&*()_[] {} \|;:',./<>?", "\n", "#cb", "\n", "#cba", "\n", "# \",\" cbaa ", "\n", "#", "\r", "#cc", "\r", "#cca", "\r\n", "# ccaa ", "\r\n" });
        lstActual = RInsight.clsRScript.GetLstLexemes("#" + "\n" + "c#" + "\n" + "ca#" + "\n" + "+#" + "\n" + "%/%#" + "\n" + "%in%#" + "\n" + @">=#~!@#$%^&*()_[]{}\|;:',./<>?#" + "\n" + " #" + "\n" + @"  #~!@#$%^&*()_[] {} \|;:',./<>?" + "\n" + "#cb" + "\n" + "#cba" + "\n" + "# \",\" cbaa " + "\n" + "#" + "\r" + "#cc" + "\r" + "#cca" + "\r\n" + "# ccaa " + "\r\n");
        Assert.Equal(lstExpected, lstActual);
    }
    ...
    public void TestGetLstTokens()
    {
        ...
        string strActual = GetLstTokensAsString(clsRScript.GetLstTokens(lstInput));
